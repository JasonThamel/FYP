...
model = YOLO('customdetection4.pt')     #Load the YOLOv8 model used for the object detection
...
def RadarThread():
    ...
    #Initialise Lists to store the predictions
    radarList = [0,0,0,0,0]      
    personList = [0,0,0,0,0,0]  
    brightnessList = [0,0,0,0,0,0]    
    disabilityList = [0,0,0,0,0,0]    
        ...
        if len(predictionsList) == 5:
            ...
            #It is adjustable to provide more or less certainty 
                ...
                if counter == 10 and mostCommonPred != "Ignore":
                    ...
                    #This sleep time values are used for demonstration
                    app.yellowLight()
...
def main():
    ...
    #Bounding boxes and Polygone zone annotations using supervision 
    boxAnnotator = sv.BoxAnnotator(thickness=1,text_thickness=1,text_scale=0.5)

    zone = sv.PolygonZone(polygon=ZONE_POLYGON, frame_resolution_wh=tuple([1280, 720]))
    zoneAnnotator = sv.PolygonZoneAnnotator(zone=zone,color=sv.Color.red(),thickness=2,text_thickness=4,text_scale=2)
    ...
    while True:
        ...
        detections = detections[detections.confidence >= CONFIDENCE/100] # Apply the confidence threshold 
        ...
        frame = boxAnnotator.annotate(scene=frame,detections=person,labels=labels)
