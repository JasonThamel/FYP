...
# Define the positions of the nodes within the window of the pygame 
...
updateInterval = 0.2  
simulationTime = 30  
edgeUpdates = pd.read_csv('edgeUpdates.csv')
...
while running:
    ...
    if currentTime - updateTime >= updateInterval:
        if timestamp < len(edgeUpdates):
            current_updates = edgeUpdates[edgeUpdates['time'] == timestamp]
            updateCSV(graph, current_updates)
    ...
    for fromNode, edges in graph.nodes.items():
        fromPosition = nodePositions[fromNode]
        for toNode, weight in edges.items():
            toPosition = nodePositions[toNode]
            pygame.draw.line(screen, edgeColor, fromPosition, toPosition, 1)
            #Find the middle position to display the time labels
            middle1 = (fromPosition[0] + toPosition[0]) // 2
            middle2 = (fromPosition[1] + toPosition[1]) // 2
            timeLabel = font.render(str(weight), True, textColor)
            screen.blit(timeLabel, (middle1, middle2))
